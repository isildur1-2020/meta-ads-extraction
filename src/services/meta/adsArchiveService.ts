...
import { printProgressMsg } from "../../lib/logs";
...
export const tryGetAdsArchive = async (
  config: AdsArchiveServiceParams & { after?: string; limit: number }
) => {
  const { limit } = config;
  printProgressMsg(`Extracting first ${counter * limit} registers`);
  const { ads, BUC, paging } = await getAdsArchiveService(config);
  console.log(BUC);
  saveOnRAM(ads);
  if (paging?.next) {
    tryGetAdsArchive({ ...config, after: paging.cursors.after });
    counter++;
  }
};

export const saveOnRAM = (ads: AdsArchiveItem[]) => {
  for (let ad of ads) {
    const { page_id, page_name } = ad;
    if (!data_companies.has(page_id)) {
      data_companies.set(page_id, page_name);
    }
  }
};

export const getAdsArchiveService = async (
  config: AdsArchiveServiceParams & { after?: string; limit: number }
) => {
  const {
    limit,
  ...
  return {
    BUC: JSON.parse(headers["x-business-use-case-usage"]),
    ads: data.data,
    paging: data.paging,
  };
};
